...
parser.add_argument("--savefilelocation","-fileloc")
projectname = "Cubic_MAPbBr3_Bandstructure"
...
def main():
    ...
    if args.outfile:
        ...
        cp2koutfile = max(filter(lambda f: f.endswith('.out'), os.listdir(os.path.abspath(os.getcwd()))), key=os.path.getctime)
...
def plotBands(energies, number_of_bands, kpoint_dis, special_kpoints, Ef, total_kpoints, project_name):
    ...
    plt.ylim(ymin, ymax)
    if args.ymin:
        plt.ylim(int(args.ymin), ymax)
    if args.ymax:
        plt.ylim(ymin, int(args.ymax))
    if int(args.ymin) & int(args.ymax):
        plt.ylim(int(args.ymin), int((args.ymax)))
    plt.plot(x_fermi,y_fermi,'k--', linewidth=1)
    plt.savefig(args.savefilelocation + '\\' + projectname)
    plt.show()
